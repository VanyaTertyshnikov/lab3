...
void PlayerInfoView::draw(std::shared_ptr<sf::RenderWindow> &window, Player &player) {
    ...
    sf::Text power(
            std::string("Power:\t") + std::to_string(player.get_primary().power),
            this->font,
            font_size);

    sf::Text ability(
            std::string("Ability:  \t") + std::to_string(player.get_primary().ability),
            this->font,
            font_size);

    sf::Text endurance(
            std::string("Endurance: \t") + std::to_string(player.get_primary().endurance),
            this->font,
            font_size);

    sf::Text bonus_damage(
            std::string("Bonus damage: \t") + std::to_string(player.get_secondary().bonus_damage),
            this->font,
            font_size);

    sf::Text bonus_defence(
            std::string("Bonus defence: \t") + std::to_string(player.get_secondary().bonus_defence),
            this->font,
            font_size);
